...
import io, os
import tempfile
# --- Import fungsi dari metode.py ---
from metode import ekstrak_pesan_lsb, embed_pesan_lsb, ekstrak_lsb_early_termination
...
# Variabel global untuk menyimpan gambar hasil sementara
gambar_hasil_sementara = None
...
# --- Fungsi untuk popup success dengan tombol download ---
def popup_success_download(nama_file, gambar_data):
    layout_popup = [
        [sg.Text("✅ EMBED SUKSES!", font=("Arial", 14, "bold"), text_color=COLOR_5)],
        [sg.Text(f"Pesan berhasil disisipkan ke dalam gambar", font=("Arial", 11))],
        [sg.Text(f"File: {nama_file}", font=("Arial", 10))],
        [sg.HorizontalSeparator()],
        [sg.Text("Klik tombol di bawah untuk menyimpan gambar:", font=("Arial", 10))],
        [sg.Button("📥 DOWNLOAD GAMBAR (JPG)", key="-DOWNLOAD-", button_color=("white", COLOR_4), size=(25, 2))],
        [sg.Button("Tutup", key="-CLOSE-", button_color=("white", COLOR_3))]
    ]
    ...
    while True:
        event_popup, _ = popup_window.read()
        if event_popup in (sg.WIN_CLOSED, "-CLOSE-"):
            break
        elif event_popup == "-DOWNLOAD-":
            # Buat nama file default untuk download (JPG)
            nama_file_default = nama_file.replace('.bmp', '.jpg').replace('.png', '.jpg')
            
            # Simpan gambar ke file yang dipilih user
            file_path = sg.popup_get_file(
                ...
                initial_file=nama_file_default  # Nama file default
            )
            ...
            if file_path:
                try:
                    ...
                    # Simpan gambar data ke file sebagai JPG
                    # Konversi dari data BMP ke JPG
                    image = Image.open(io.BytesIO(gambar_data))
                    image.save(file_path, "JPEG", quality=95)
                    
                    sg.popup_ok(f"Gambar berhasil disimpan sebagai JPG!\n\nLokasi: {file_path}")
                    break
                except Exception as e:
                    sg.popup_error(f"Gagal menyimpan gambar: {str(e)}")
...
while True:
    ...
    if event == "-PROSES-":
        ...
        nama_file_output = f"Gambar_Embedded_{timestamp}.jpg"
        path_output = os.path.join("hasil", nama_file_output)
        
        try:
            # Proses embedding langsung ke JPG
    ...
    if event == "-PROSESEKSTRAK-":
        ...
        berhasil, hasil_ekstrak = ekstrak_lsb_early_termination(Path_gambar_ekstrak)
